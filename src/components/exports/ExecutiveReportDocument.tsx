...
export default function ExecutiveReportDocument({
  ...
  const candidateNameText = candidateName ?? t('report.candidateName')
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        >
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: '56px 1fr', gap: '12px', alignItems: 'center' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '12px', color: '#7a6455', marginTop: '4px', fontFamily: 'Arial, sans-serif', letterSpacing: '0.08em', textTransform: 'uppercase' }}>{t('report.boardSummary')}</div>
              </div>
            </div>
            ...
            <div style={{ marginTop: '12px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              ...
              >
                {t('report.completionDate')}: {generatedAt}
              </span>
              <span
                style={{
                  borderRadius: '999px',
              ...
              >
                {t('report.completionScore')}: {completionScore}%
              </span>
              <span
                style={{
                  borderRadius: '999px',
              ...
              >
                {t('report.documentVersion')}: {reportCopy.documentVersion}
              </span>
            </div>
          </div>
          ...
          >
            ...
            <div style={{ marginTop: '14px', borderRadius: '16px', background: 'radial-gradient(circle at top, #fffaf6 0%, #f8efe8 100%)', padding: '10px', border: '1px solid #efe1d0' }}>
              <svg viewBox="0 0 220 220" width="100%" height="220" aria-label={t('report.radarChartLabel')} role="img">
        ...
        >
          <div style={{ fontSize: '12px', fontWeight: 700, color: '#4c3a2e' }}>{t('report.closingHeading')}</div>
          <div style={{ lineHeight: 1.6 }}>
            {t('report.closingBody')}
          </div>
          <div style={{ lineHeight: 1.6 }}>
            <strong>{t('report.followLink')}</strong> <a href="https://www.linkedin.com/in/joaocosta1695/" style={{ color: '#8b5e3c', textDecoration: 'none' }}>{t('report.followLinkText')}</a>
          </div>
          <div style={{ lineHeight: 1.6 }}>
            <strong>{t('report.buyCoffee')}</strong> {t('report.buyCoffeeBody')}
          </div>
          <span style={{ color: '#9b8570' }}>{t('report.tagline')}</span>
        </footer>
      </div>
    </div>
  )
