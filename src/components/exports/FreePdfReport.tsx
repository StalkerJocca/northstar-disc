...
const lockedSections = [
  ['Detailed Behavioral Breakdown', 'Explore how your behavioral blend changes your pace, decisions, and influence across a wider range of professional situations.'],
  ['Stress Profiles', 'See likely pressure triggers, early warning signs, and practical recovery strategies tailored to your DISC pattern.'],
  ['Team Dynamics', 'Understand the conditions that help you collaborate, resolve friction, and build confidence with complementary styles.'],
  ['Communication Guides', 'Get situation-specific language, meeting habits, and feedback approaches for stronger working relationships.'],
]
...
export default function FreePdfReport({ profile, primaryTrait, secondaryTrait, generatedAt, labels }: FreePdfReportProps) {
  ...
  return <Document title={`${labels.brand} — ${labels.title}`}><Page size="A4" style={styles.page}>
    ...
    <Text style={styles.footer}>{labels.brand} · Free profile snapshot</Text>
    ...
    <Text style={styles.cta}>Unlock the complete Executive DISC Analysis at northstardisc.com</Text><Text style={styles.footer}>{labels.brand} · Preview only</Text>
  </Page></Document>
}
