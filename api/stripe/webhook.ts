...
const planForPrice = (priceId: string | null) => priceId === process.env.STRIPE_ENTERPRISE_WHITE_LABEL_PRICE_ID ? 'enterprise' : priceId === process.env.STRIPE_TEAM_ANALYSIS_PRICE_ID ? 'team' : priceId === process.env.STRIPE_EXECUTIVE_REPORT_PRICE_ID ? 'executive' : null
...
async function syncSubscription(input: { userId: string; customerId: string | null; subscriptionId: string | null; sessionId?: string; priceId: string | null; status: string }) {
  ...
  if (!plan) throw new Error(`Received subscription event for an unknown Stripe price: ${input.priceId ?? 'missing'}`)
...
async function handleRequest(request: Request): Promise<Response> {
  ...
  try {
    const admin = getSupabaseAdmin()
    const { data: claimed, error: claimError } = await admin.rpc('claim_stripe_webhook_event', { p_event_id: event.id, p_event_type: event.type })
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (userId && session.metadata?.product === 'northstar_coach_credits' && session.payment_status === 'paid') {
        ...
        const { error: creditError } = await admin.rpc('apply_coach_credit_purchase', { p_user_id: userId, p_session_id: session.id, p_credits: credits })
    ...
    if (event.type === 'customer.subscription.updated' || event.type === 'customer.subscription.deleted') {
      const subscription = event.data.object as Stripe.Subscription; const { data } = await admin.from('subscriptions').select('user_id').eq('stripe_subscription_id', subscription.id).maybeSingle()
    ...
    const { error: completeError } = await admin.rpc('complete_stripe_webhook_event', { p_event_id: event.id })
    if (completeError) throw completeError
  } catch (error) {
    console.error('[Stripe Webhook Error]:', error)
    try { await getSupabaseAdmin().rpc('fail_stripe_webhook_event', { p_event_id: event.id, p_error: error instanceof Error ? error.message.slice(0, 1000) : 'Webhook processing failed.' }) } catch (logError) { console.error('[Stripe Webhook Event Log Error]:', logError) }
