...
export default async function handler(request: Request): Promise<Response> {
  ...
  if (action === 'invite') {
    ...
    const { data, error } = await admin.from('coach_clients').insert({ user_id: user.id, client_name: name, client_email: email }).select('invite_token').single()
    if (error || !data) {
  ...
  if (action === 'complete') {
    ...
    const { data: invite } = await admin.from('coach_clients').select('id, client_email').eq('invite_token', body.inviteToken).maybeSingle()
